...
public:
  ...
  LogicalResult matchAndRewrite(AtenRepeatOp op,
    ...
    if (!getListConstructElements(op.repeats(), repeats))
      return rewriter.notifyMatchFailure(op, "Unimplemented: repeats not list of Scalar");
    ...
    for (int i = 0; i < rank; i++) {
      ...
      Value scaledSize = rewriter.create<AtenMulIntOp>(
          loc, dimSize, scale);
    ...
    Type reshapeType0 = ValueTensorType::get(
        context, llvm::makeArrayRef(new_shape0), dtype);
    Type reshapeType1 = ValueTensorType::get(
        context, llvm::makeArrayRef(new_shape1), dtype);
    Type reshapeType2 = ValueTensorType::get(
        context, llvm::makeArrayRef(new_shape2), dtype);

    std::cout << "GTY: 3" << std::endl;
    auto listType = Torch::ListType::get(Torch::IntType::get(op.getContext()));
    Value new_dims0 = rewriter.create<PrimListConstructOp>(
        loc, listType, new_sizes0);
    Value new_dims1 = rewriter.create<PrimListConstructOp>(
        loc, listType, new_sizes1);
    Value new_dims2 = rewriter.create<PrimListConstructOp>(
        loc, listType, new_sizes2);
    auto reshaped = rewriter.create<AtenViewOp>(loc, reshapeType0, op.self(), new_dims0);
    auto expanded = rewriter.create<AtenBroadcastToOp>(loc, reshapeType1, reshaped, new_dims1);

    std::cout << "GTY: 4" << std::endl;
    rewriter.replaceOpWithNewOp<AtenViewOp>(op, op.getType(), expanded, new_dims2);
    return success();
  }
};
} // namespace
